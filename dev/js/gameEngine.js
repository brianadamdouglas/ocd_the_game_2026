			...
			*/
			function init(){
				addStartScreen();// i am placing this first in the execution list beacuse I need these elements to install first.
				addPlayer();
				addRotater();
				addStage();
...
*/
			...
			*/			
			function addStage(){		
				...
				var className = "stage";
				g_stage = new Stage();
				g_stage.init(container, spriteID, className, g_stageStartX, g_stageStartY, g_stageWidth, g_stageHeight);
			...
			*/			
			function createSprite(id){
				...
				//console.log([id,classAcronym]); use to get tile# / tile type relationship[
				...
				var listener1 = g_gameboard[id].listener1;
				var listenerString1 = g_gameboard[id].listener1;
				var listener2 = g_gameboard[id].listener2;
				var listenerString2 = g_gameboard[id].listener2;
				...
				var newTile = g_stage.addTile(spriteID, className, x, y,w,h,imagePath, listener, listenerString, thoughtType, visibility, g_stageSpriteArray, stickyHoldingOffset, IDOverride, dropTargetFunction, moveObject, listener1, listener2,listenerString1, listenerString2);			
...
/* ------------------------------   INTERACTION FUNCTIONS ---------------------------------*/			
			...
			*/
			function pickUpItem(target){ //boolean
				...
				if(target == undefined){
					...
					var results = checkForInteraction(target);
				}
				if(results.length > 0){
					for(var i = 0; i< results.length; i++){
						var spriteToCheck = results[i].sprite;
			...
			function interactWithStationaryItem(target){ //boolean
				...
				if(results.length > 0){
					for(var i = 0; i< results.length; i++){
						...
						e_InteractionExtendedActions(interactedObject);
					}
				}
			...
			*/
			function checkForInteraction(target){ //boolean
