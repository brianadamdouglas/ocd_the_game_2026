var Main_Controller = Controller.extend({
  /**
  * Constructor
...
construct: function() { 
		...
		this._currentTile;
		this._startMatrix;
		this._potentialThoughtArray = new Array();
		this._preTriggeredControls = new Array();
		this._mainModel;
		this._walkInterval;
  },
...
addListners:function(){
	...
	g_eventHandler.addAListener("pickUpItem", this);
	
	
	g_eventHandler.addAListener("startWalking", this);
	g_eventHandler.addAListener("walkForward", this);
	g_eventHandler.addAListener("walkForwardJoystick", this);
	g_eventHandler.addAListener("turnLeft", this);
	g_eventHandler.addAListener("turnRight", this);
	g_eventHandler.addAListener("turnAround", this);
	g_eventHandler.addAListener("stopWalking", this);
	g_eventHandler.addAListener("buttonA_Action", this);
	g_eventHandler.addAListener("buttonB_Action", this);
...
addStartMatrix: function(matrix){
...
buttonB_Action:function(){
	this.pickUpItem();	
},


buttonA_Action:function(){
...
walkForwardJoystick:function(){
	clearInterval(this._walkInterval);
	this.walkForward();
	//g_eventHandler.dispatchAnEvent("startWalk",{});
	this._walkInterval = setInterval(this.walkIntervalFunction.bind(this), 90); // use .bind to correct scope
},

walkIntervalFunction:function(){
...
*/			
resetPosition: function(hitObstacle,type,previousMoves){  //function(hitObstacle,previousMoves){//array
...
*/
stickyObjectLifted: function(data){
...
* @param {StickyTile} stickyObject
*/
stickyObjectDropped: function(data){
...
checkForDropTarget: function(stickyObject){
	var results = this.checkForInteraction(undefined,10);
	if(results.length > 0){
...
pickUpItem: function(target){ //boolean
	...
	if(target == undefined){
		...
		var results = this.checkForInteraction(target,10);
	}
	if(results.length > 0){
		for(var i = 0; i< results.length; i++){
			var spriteToCheck = results[i].sprite;
...
interactWithStationaryItem: function(target){ //boolean
	...
	if(target == undefined){
		var results = this.checkForInteraction(undefined,20);	
	}else{
		var results = this.checkForInteraction(target,20);
	}
	
	if(results.length > 0){
		var doorTriggered = false; // need to put this in because with the widened interaction radius, it causes some items that mutually listen to both fire, negating each other
		for(var i = 0; i< results.length; i++){
			doorTriggered = this.interactionResponse(results[i].sprite,results[i].direction, doorTriggered);
...
*/
checkForInteraction: function(target, catchableDistance){ //boolean
	...
	/* 
	var playerRect = {top:0, right:0, bottom:0, left:0};
	playerRect.left = Number(playerTransformedRect.getRect().left);
	playerRect.right = Number(playerTransformedRect.getRect().right);
	playerRect.top = Number(playerTransformedRect.getRect().top);
	playerRect.bottom = Number(playerTransformedRect.getRect().bottom); */
	...
	*/
	...
	for(var i = 0; i<predictedObjectArray.length; i++){
		...
		/* var targetRect = {top:0, right:0, bottom:0, left:0};
		targetRect.left = Number(spriteToCheck.getRect().left);
		targetRect.right = Number(spriteToCheck.getRect().right);
		targetRect.top = Number(spriteToCheck.getRect().top);
		targetRect.bottom = Number(spriteToCheck.getRect().bottom); */
		...
		switch(this._stageRotation){
				case 0:
					//targetRect.left = targetRect.left - 30; //Added 3/5/2016
					//targetRect.right = targetRect.right + 30; //Added 3/5/2016
					/* console.log("start 0");
					console.log(targetRect);
					console.log(playerRect);
					console.log("end 0"); */
					if((targetRect.bottom >= playerRect.top - catchableDistance && targetRect.bottom <= playerRect.bottom) && (targetRect.left >= playerRect.left - grabDistance && targetRect.right <= playerRect.right + grabDistance)){
						direction = "UP";
				...
				case 90:
					...
					console.log(targetRect);
					console.log(playerRect);
					console.log(targetRect.right>= playerRect.left - catchableDistance && targetRect.right <= playerRect.right);
					console.log(targetRect.top >= playerRect.top - grabDistance && targetRect.bottom <= playerRect.bottom + grabDistance);
					console.log("end 90"); */
					//targetRect.top = targetRect.top - 30; //Added 3/5/2016
					//targetRect.bottom = targetRect.bottom + 30; //Added 3/5/2016
					if((targetRect.right>= playerRect.left - catchableDistance && targetRect.right <= playerRect.right ) && (targetRect.top >= playerRect.top - grabDistance && targetRect.bottom <= playerRect.bottom + grabDistance)){
						direction = "LEFT";
						possibleInteraction = true;
					}
					break;
				case 180:
					/* console.log("start 180");
					console.log(targetRect);
					console.log(playerRect);
					console.log("end 180"); */
					//targetRect.left = targetRect.left - 30; //Added 3/5/2016
					//targetRect.right = targetRect.right + 30; //Added 3/5/2016
					if((targetRect.top <= playerRect.bottom + catchableDistance && targetRect.top > playerRect.bottom) && (targetRect.left >= playerRect.left - grabDistance && targetRect.right <= playerRect.right + grabDistance)){
							direction = "DOWN";	
							possibleInteraction = true;
					}
					break;
				case 270:
					/* console.log("start 270");
					console.log(targetRect);
					console.log(playerRect);
					console.log(targetRect.left<= playerRect.right + catchableDistance && targetRect.left > playerRect.left);
					console.log("end 270"); */
					//targetRect.top = targetRect.top - 30; //Added 3/5/2016
					//targetRect.bottom = targetRect.bottom + 30; //Added 3/5/2016
					if((targetRect.left<= playerRect.right + catchableDistance && targetRect.left > playerRect.left) && (targetRect.top >= playerRect.top - grabDistance && targetRect.bottom <= playerRect.bottom + grabDistance)){
						direction = "RIGHT";
						possibleInteraction = true;
					}
					break;
...
* @param {bool} Boolean 
*/
setCanMoveForward: function(data){
...
*/	
interactionResponse: function(spriteToCheck, direction, doorTriggered){
	console.log(spriteToCheck);
	console.log(spriteToCheck.getViewDivClass("door"));
	console.log(spriteToCheck.getViewVisibility());
	if(spriteToCheck.getViewDivClass("door") && spriteToCheck.getViewVisibility() & !doorTriggered){
...
*/		
interactionExtendedActions: function(interactedObject, intensity){
...
*/			
dropTargetHamper: function(data){
...
*/			
dropTargetBookcase: function(data){
	var droppedItemController = data.droppedItemController;
...
addOCD_control: function(){//NOT PART OF CORE
	this._MIND = new MIND();
...
addVolumeControl: function(audioInstance){
	var volumeControlInfo = this._mainModel.getVolumeControlInfo();
	var classAcronym = volumeControlInfo.type;
	var diffX = (g_screenWidth - volumeControlInfo.w) - 10;
	var diffY = 0//(g_screenHeight - volumeControlInfo.h) - 5;
	var data = {
		className:this._mainModel.getGameboardClasses()[classAcronym],	
		imagePath :this._mainModel.getGameboardImageLookup()[classAcronym],	
		id:'volumeControl',
...
addJoystick:function(){
	var screenInfo = g_gameboardModel.getJoystickElements();
	var data = {
		className:"joystick",	
		id:'joystick',
		container:"body",	
	...
	this._joystick = new Joystick_Controller();
	var newView = new Combination_View();
	this._joystick.bindView(newView,data);
	this._joystick.hide();
	...
	for(var i = 0; i< screenInfo.length; i++){
		...
		var dataNested = {
			...
			h:screenInfo[i].h, 
			imgs:this._mainModel.getGameboardImageLookup()[classAcronym], 
			downFunction:screenInfo[i].downFunction, 
			pressFunction:screenInfo[i].pressFunction, 
			releaseFunction:screenInfo[i].releaseFunction,
			IDOverride:screenInfo[i].IDOverride
		};
		
		
		this._joystick.addTile(dataNested);
		
		
	} 
	
	
	this._joystick.getInterfaceElement("joystickUp").getView().show();
	this._joystick.getInterfaceElement("joystickLeft").getView().show();
	this._joystick.getInterfaceElement("joystickRight").getView().show();
	this._joystick.getInterfaceElement("joystickDown").getView().show();
	this._joystick.getInterfaceElement("buttonA").getView().show();
	this._joystick.getInterfaceElement("buttonB").getView().show();
...
gameOver:function(){
	...
	if(rectToCheck.bottom < 130 && rectToCheck.left > 850 && rectToCheck.right < 1020){
...
/* 	console.log(data.type);
	...
	var rectToCheck = this.getPlayerTransformRect();
	if(rectToCheck.bottom < 150){//if(data.type == "frontDoor" && rectToCheck.bottom < 150){
		this.gameOver();
	}
},
