...
*/
var Stage_Controller = Tile_Controller.extend({
construct: function() { 
	this.SC.construct();
...
repositionStage:function(targetRect, playerRect, stageRotation, previousMoves){ 
 ...
 switch(stageRotation){
					case 0:
						//if target is above
						if(targetRect.bottom >= playerRect.top && playerRect.bottom > targetRect.top && targetRect.bottom - playerRect.top <= maximumOverlay){
						...
						//if target is below	
							...
							if(canMove){
								var distance = ((playerRect.bottom + (padding)) -  targetRect.top); // had been *2
								this.moveStageSansCallback(0,distance);
						...
						// if target is to the left	
							...
							this.movementPossible(true);
							if(canMove){
								var distance = (playerRect.left -  (targetRect.right + (padding * 2)));
						...
						}else if(targetRect.left <= playerRect.right && playerRect.left < targetRect.right && playerRect.right - targetRect.left <= maximumOverlay){
							var canMove = this.checkPreviousMoves(previousMoves, 4);
							this.movementPossible(true);
							if(canMove){
								var distance = ((playerRect.right + (padding * 2)) -  targetRect.left);
								this.moveStageSansCallback(distance,0);
								this.movementPossible(true);
							}
						...
						break;
					case 90:
						if(targetRect.right >= playerRect.left && playerRect.right > targetRect.left && targetRect.right - playerRect.left <= maximumOverlay ){
							var canMove = this.checkPreviousMoves(previousMoves, 1);
							if(canMove){
							...
							this.movementPossible(true);
							if(canMove){
								var distance = ((playerRect.right + (padding)) - targetRect.left );//had been *2
								this.moveStageSansCallback(distance,0);
								this.movementPossible(true);
							}
						...
						//player to the left of the object
						}else if(targetRect.bottom >= playerRect.top && playerRect.bottom > targetRect.top && targetRect.bottom - playerRect.top <= maximumOverlay){
							var canMove = this.checkPreviousMoves(previousMoves, 3);
							this.movementPossible(true);
							if(canMove){
								var distance = (playerRect.top - (targetRect.bottom + (padding * 2)));
								this.moveStageSansCallback(0,distance);
								this.movementPossible(true);
							}
							...
							return 3;
						//player to the right
						}else if(targetRect.top <= playerRect.bottom && playerRect.top < targetRect.bottom && playerRect.bottom - targetRect.top <= maximumOverlay ){
							var canMove = this.checkPreviousMoves(previousMoves, 4);
							this.movementPossible(true);
							if(canMove){
								var distance = ((playerRect.bottom + (padding * 2)) - targetRect.top);
								this.moveStageSansCallback(0,distance);
								this.movementPossible(true);
							}
					...
					case 180:
						if(targetRect.bottom >= playerRect.top && playerRect.bottom > targetRect.top && targetRect.bottom - playerRect.top <= maximumOverlay){
							...
							this.movementPossible(true);
							if(canMove){
								var distance = (playerRect.top - (targetRect.bottom + (padding)));// had been *2
								this.moveStageSansCallback(0,distance);
								this.movementPossible(true);
							}
						...
						break;
					case 270:
						if(targetRect.left <= playerRect.right && playerRect.left < targetRect.right && playerRect.right - targetRect.left <= maximumOverlay){
							var canMove = this.checkPreviousMoves(previousMoves, 1);
							if(canMove){
							...
							this.movementPossible(true);
							if(canMove){
								var distance = (playerRect.left- (targetRect.right  + (padding)));// had been * 2
								this.moveStageSansCallback(distance,0);
								this.movementPossible(true);
							}
