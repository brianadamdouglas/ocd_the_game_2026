...
init: function(model){
		...
		this.addListners();
		this.addPlayer(g_playerInfo);
		this.addRotater(g_rotaterX, g_rotaterY, g_stageWidth, g_stageHeight);
		this.addStage(g_stageStartX, g_stageStartY, g_stageWidth, g_stageHeight);
		this.createHitTestQuadrants();
		this.addStartMatrix(g_thoughtMatrix);
		this.addPotentialThoughtArray(g_potentialThoughtArray);
		this.buildStage(g_gameboard);
},
...
addPlayer:function (playerInfo){
		...
		var data = {
			...
			imgs:gameBoardImageLookup[classAcronym],
			id:'player',
			container:"body",
			x:playerInfo.x,
			y:playerInfo.y,
...
turnOnRandomizedItems:function(){
	for(var i = 0; i<this._preTriggeredControls.length; i++){
		this.interactionResponse(this._startMatrix[this._preTriggeredControls[i]], "up");
	}
},
...
createSprite: function (id){
		...
		var startFrame = (g_gameboard[id].startFrame == undefined) ? 0 : g_gameboard[id].startFrame
		var data = {
			className:gameBoardClasses[classAcronym],	
			imgs:gameBoardImageLookup[classAcronym],
			...
			objectType:g_gameboard[id].objectType,
...
*/			
resetPosition: function(hitObstacle,previousMoves){//array
	...
	var targetName = hits[0]._classReference.getViewDIV();//using for debugging, remove later
...
pickUpItem: function(target){ //boolean
	...
	if(this._player.getIsHoldingObject()){
		...
		this.movementProgress();
		this._player.setIsHoldingObject(false);
		this.interactionExtendedActions(this._activeStickyObject);
...
checkForHit: function (){
	...
	if(hitObstacle[0]){
		...
		for(var i = 0; i<hits.length ; i++){ // updated 8/20 so that it checks all items of possible interaction
			var isObstacle = hits[i]._classReference.getViewDIV().hasClass( "obstacle" );
			if(isObstacle){	
				this._canMoveForward = false;
				previousMoves.push(this.resetPosition(hitObstacle, previousMoves));
...
interactionExtendedActions: function(interactedObject){
	...
	this._MIND.addThought(interactedObject.getViewID(),thoughtType,objectType,2, this.getPlayerLocation());
