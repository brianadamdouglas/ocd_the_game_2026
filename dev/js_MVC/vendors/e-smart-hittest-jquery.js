  ...
  */
  $.fn.objectHitTest = function(options) {
   ...
   if(settings.object == null) // si the object to test is not set
    ...
    var rectsInstersects = objectRect.intersects(objectToTestRect);//BD mod
...
function Rectangle(x, y, width, height) {
    ...
    this.rectContainsPoint = function (pointX, pointY){
        ...
        console.log({top:this.y,right:this.x+this.width,bottom:this.y+this.height,left:this.x}); */
	 ...
	 */
	 /*
	 I modified this code based onthe rotation of the stage. There are four possibilites. The rects are all absolute so when the <rotater> dive spins, all the
	 <tiles> in the <stage> have a new relationship to the (0,0) point at the top of the browser. Because the <player> is fixed the player's relationship to a hit 
	 is altered because the rects are in a different configuration.
	 */
	this.intersects = function(rect){
			// modify this based on container rotation BD	
			
			switch(g_mainGameController.getStageRotation()){
						case 0:
							return (this.x <= rect.x + rect.width && rect.x <= this.x + this.width && this.y <=rect.y + rect.height && rect.y <= this.y + this.height);
							break;
						case 90:
							return (this.x <= rect.x + rect.height && rect.x <= this.x + this.width && this.y <=rect.y + rect.width && rect.y <= this.y + this.height);
							break;
						case 180:
							return (this.x <= rect.x + rect.width && rect.x <= this.x + this.width && this.y <=rect.y + rect.height && rect.y <= this.y + this.height);
							break;
			...
			// modify this based on container rotation BD
		// This Was the original codereturn (this.x <= rect.x + rect.width && rect.x <= this.x + this.width && this.y <=rect.y + rect.height && rect.y <= this.y + this.height);
	}
	/*
