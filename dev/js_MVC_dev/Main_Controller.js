	...
	for(var i = 0; i<predictedObjectArray.length; i++){
		...
		switch(this._stageRotation){
				case 0:
					...
					if((targetRect.bottom >= playerRect.top - catchableDistance && targetRect.bottom <= playerRect.bottom) && (targetRect.right >= playerRect.left - grabDistance && targetRect.left <= playerRect.right + grabDistance)){
						direction = "UP";
				...
				case 90:
					...
					if((targetRect.right>= playerRect.left - catchableDistance && targetRect.right <= playerRect.right ) && (targetRect.top <= playerRect.bottom + grabDistance  && targetRect.bottom >= playerRect.top - grabDistance)){
						direction = "LEFT";
						possibleInteraction = true;
					}
					break;
				case 180:
					//targetRect.left = targetRect.left - 30; //Added 3/5/2016
					//targetRect.right = targetRect.right + 30; //Added 3/5/2016
					if((targetRect.top <= playerRect.bottom + catchableDistance && targetRect.top > playerRect.bottom) && (targetRect.right >= playerRect.left - grabDistance && targetRect.left <= playerRect.right + grabDistance)){
							direction = "DOWN";	
							possibleInteraction = true;
					}
					break;
				case 270:
					//targetRect.top = targetRect.top - 30; //Added 3/5/2016
					//targetRect.bottom = targetRect.bottom + 30; //Added 3/5/2016
					if((targetRect.left<= playerRect.right + catchableDistance && targetRect.left > playerRect.left) && (targetRect.top <= playerRect.bottom + grabDistance && targetRect.bottom >= playerRect.top - grabDistance)){
						direction = "RIGHT";
						possibleInteraction = true;
					}
					break;
...
addVolumeControl: function(audioInstance){
	...
	var diffX = (g_screenWidth - volumeControlInfo.w) - 5;
	var diffY = (g_screenHeight - volumeControlInfo.h) - 5;
	var data = {
		className:this._mainModel.getGameboardClasses()[classAcronym],	
		imagePath :this._mainModel.getGameboardImageLookup()[classAcronym],	
		id:'volumeControl',
